...
//set date for the doc
$_GET ['datenum'] = '10';
$_GET ['monthnum'] = '06';
$_GET ['monthname'] = 'december';
$_GET ['yearnum'] = '2017';

//Get event uri list fof a date from eventregistry.com
$dateStart = $_GET ['yearnum'].'-'.$_GET ['monthnum'].'-'.$_GET ['datenum'];
$dateEnd = $dateStart;
$lang = 'eng';
$minArticlesInEvent=200;
$resultType=uriList;
$action = 'getEvents';
$event_registry_api_key = '201605ee-2f82-4037-94f2-80d2641994ae';

   $key = array('action', 'dateStart', 'dateEnd',  'resultType','minArticlesInEvent', 'lang', 'apiKey');
   $value = array($action, $dateStart, $dateEnd, $resultType, $minArticlesInEvent, $lang, $event_registry_api_key); 
  ...
  $total_count = $remote_data ['uriList']['count']; // echo $total_count;
  
  $eventUriarray = $remote_data ['uriList']['results'];  
  
  // start loop to get event details by eventUri
  $i = 0;
...
// Create data to send to curl function for event registry.com  
    ...
    //$eventsCount=1;
    $action = 'getEvent';
    $eventUri = $eventUriarray[$i];
    $resultType = 'info';
    // $infoIncludeEventSummary = 'true';
    $infoIncludeEventConcepts = 'false';
    //$infoIncludeEventArticleCounts = 'false';
    $callback = 'JSON_CALLBACK';
   $key = array('action', 'eventUri',  'resultType','infoIncludeEventConcepts', 'apiKey');
   $value = array($action, $eventUri, $resultType, $infoIncludeEventConcepts, $event_registry_api_key); 
   $url = 'http://eventregistry.org/json/event';
   $num_param = 5;
   $method = 'GET';
  ...
  if ($remote_data [$eventUri]['info']['title']['eng'] != NULL && $remote_data [$eventUri]['info']['location']['label']['eng'] != NULL ){
