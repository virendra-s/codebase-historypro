...
EOF;
		...
		} // Normal processing if statement
	...
	
	{
		...
		if(verifychecksum_e($_POST,$paytm_merchant_key,$_POST['CHECKSUMHASH']) === "TRUE"){
			...
			if($_POST['RESPCODE'] =="01"){
				...
				$msg= "Thank you for your order $current_user->user_firstname. Your transaction has been successful.";
				echo '<pre>'; print_r($order); echo '</pre>';				
				$result = pmpro_changeMembershipLevel($level_id, $current_user->ID);
				
						if ($result){
								 $msg.= "Your membership has changed to $level_id for Rs.$amount. You can now enjoy the paid video.";
								 }
							else 
							{
							$msg.= "Your Membership level has not changed and remains at $level_id.";
							} 
				
				
				
				
				$msg.= "You can now enjoy the paid video. $result $paytm_callback";
                // wp_new_user_notification($user_id);

				// Send email to user with image
				$body = '<html><body><img src="https://www.webtalkies.in/wp-content/uploads/2016/10/Thank-you_01-01.jpg" /></body></html>';
				$subject = 'Exclusive Episode Of Maid In India';
				...
				$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "paytm_donation SET payment_status = 'Failed Payment' WHERE  id = %d", $_POST['ORDERID']));
			}
			...
			$msg= "Security error!"; echo '<pre>'; print_r($order); echo '</pre>';
