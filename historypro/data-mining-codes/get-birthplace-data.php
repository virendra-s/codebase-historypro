...
//Pick a Birthday entry
$newline = "SELECT eventName FROM hpro_events_wiki_december_13 WHERE eventCategory = 'Birth'";

$person_names = $con->get_results($newline);

// DEBUG : foreach ($person_names as $person) {echo $person->eventName.'<br/>';};

//---------------WORKS TILL HERE-------------

//Pick one name nad query diffbot
foreach ($person_names as $person) { 
    
    $birthplace = hpro_get_birthplace_from_diffbot($person->eventName);
    echo $birthplace;
    exit();
...
//Function to get birthplace from diffbot
function hpro_get_birthplace_from_diffbot($person){
...
// Create data to send to curl function for creating curl params for wiki request   
    $google_search_api_key = 'AIzaSyAhAjfvzG0OIoGQnnWDKSDdJ7hwML93qRA';
    $cx = '014265745325216157685:utnbolmrf9m';
   $key = array('key', 'cx', 'q');
   $value = array($google_search_api_key, $cx, 'Birthplace+'.$person); 
   $url = 'https://www.googleapis.com/customsearch/v1';
   $num_param = 3;
   $method = 'GET';
    }
    else{
   // Create data to query thorugh diffbot
   $query_url = 'https://www.google.co.in/search?q=Place+of+Birth+'.$person;
   $diffbot_token = 'ec45908d5610e2dbc03676d894c8b358'; 
   $key = array('token', 'url');
    $value = array($diffbot_token, urlencode($query_url)); 
 ...
 $birthplace = explode('Map of ', $remote_data['objects'][0]['images'][1]['title']);
 echo $birthplace[1]; 
